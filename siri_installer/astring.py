...
def main():
    ...
    if Sonuc == "2":
        ...
        if soup.title.string == "Create new application":
            ...
            app_title = soru("APPın Adı ne olsun? (Otomatik Oluşturmak içinn enter düğmesine basın): ")
            if app_title == '':
            ...
            app_shortname = soru("APPın kısa adı ne Olsun? (Otomatik Oluşturmak için enter düğmesine basın) \[5-32 karakter\]: ")
            if app_shortname == '':
            ...
            if app == "ERROR":
                hata("(!) Telegram otomatik app oluşturma engellendi. Scripti yeniden başladın.")
                exit(1)
